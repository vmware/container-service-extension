...
class PluginManager:
    ...
    def register(self, pluginDir):
        ...
        transferRequest = json.dumps({
            ...
            "size": os.stat(os.path.join(pluginDir, 'dist', 'container-ui-plugin.zip')).st_size
        })
        ...
        response = self._session.put(transferUrl, data=open(os.path.join(pluginDir, 'dist', 'container-ui-plugin.zip'), 'rb'))
...
def main():
    ...
    if not pathlib.Path('manage_plugin.json').exists():
