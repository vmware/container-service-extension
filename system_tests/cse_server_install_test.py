...
from container_service_extension.system_test_framework.utils import \
    yaml_to_dict, dict_to_yaml_file, diff_amqp_settings, restore_customizaton_scripts
...
class CSEServerInstallationTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls._vdc = get_vdc(cls._client, cls._config['broker']['vdc'], org=cls._org)
    ...
    def test_0010_config_invalid_keys(self):
        ...
        for config_dict in configs:
            ...
            try:
                ...
                print(f"{self._active_config_filepath} passed validation when it should not have")
                assert False
            except KeyError:
                pass
    ...
    def test_0030_config_valid(self):
        ...
        try:
            ...
            print(f"{self._active_config_filepath} did not pass validation when it should have")
            assert False
    ...
    def test_0050_install_no_capture(self):
        ...
        command: cse install --config cse_test_config.yaml --template photon-v2
            --amqp skip --ext skip --ssh-key ~/.ssh/id_rsa.pub --no-capture
        required files: ~/.ssh/id_rsa.pub, cse_test_config.yaml, photon-v2 init,
    ...
    def test_0080_install_cleanup_true(self):
        ...
        try:
            check_cse_installation(self._config, check_template=PHOTON_TEMPLATE_NAME)
        except EntityNotFoundException:
