    ...
    for template_config in config['broker']['templates']:
        ...
        try:
            ...
            if 'photon' in temp_vapp_name:
                script = \
                    install_utils.get_data_file(env.STATIC_PHOTON_CUST_SCRIPT)
                ...
                script = \
                    install_utils.get_data_file(env.STATIC_UBUNTU_CUST_SCRIPT)
                pattern = \
                    r'((kubernetes|docker\S*|kubelet|kubeadm|kubectl)\S*=\S*)'
