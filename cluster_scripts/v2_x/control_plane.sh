...
if [ "$is_tkgm" = true ]; then
  ...
  sed -i 's/VCD_HOST/{vcd_host}/; s/ORG/{org}/; s/OVDC/{ovdc}/; s/OVDC_NETWORK/{ovdc_network}/; s/VIP_SUBNET_CIDR/{vip_subnet_cidr}/; s/CLUSTER_ID/{cluster_id}/' $vcloud_configmap_path
fi
