...
\b
        cse run config.yaml
            ...
            of the config happens using the password provided by the user thru
...
\b
        cse run config.yaml --skip-check
            ...
            password provided by the user thru stdin.
\b
...
\b
    cse template install config.yaml
        ...
        be decrypted using the password provided by the user thru stdin.
\b
...
def decrypt(ctx, input_file, output_file):
    ...
    password = os.getenv('CSE_CONFIG_PASSWORD') or prompt_text(
        'Password', hide_input=True, color='green')
    try:
        check_python_version(ConsoleMessagePrinter())
