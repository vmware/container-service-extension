...
class DefEntityPhase:
    ...
    @staticmethod
    def get_operation(phase: str) -> DefEntityOperation:
        if not phase:
            return DefEntityOperation.UNKNOWN
        try:
            return DefEntityOperation[phase.split(':')[0]]
        except Exception:
            return DefEntityOperation.UNKNOWN

    @staticmethod
    def get_operation_status(phase: str) -> DefEntityOperationStatus:
        if not phase:
            return DefEntityOperationStatus.UNKNOWN
        try:
            return DefEntityOperationStatus[phase.split(':')[1]]
        except Exception:
            return DefEntityOperationStatus.UNKNOWN

    @staticmethod
    def is_operation_status_success(phase: str) -> bool:
        try:
