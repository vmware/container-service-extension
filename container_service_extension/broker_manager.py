...
class BrokerManager(object):
    ...
    def invoke(self, op, on_the_fly_request_body=None):
        ...
        if op == Operation.INFO_OVDC:
            ...
            pks_ctx, ovdc = self._create_pks_ctx()
            if self.body[CONTAINER_PROVIDER] == CtrProvType.PKS.value:
                self._create_compute_profile(pks_ctx)
            task = self.ovdc_cache.\
                set_ovdc_container_provider_metadata\
                (ovdc, pks_ctx, self.body[CONTAINER_PROVIDER])
            response_body = {}
    ...
    def _create_compute_profile(self, pks_ctx):
        ovdc_id = self.body.get('ovdc_id')
        org_name = self.body.get('org_name')
