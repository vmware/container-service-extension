...
def should_configure_amqp(client, amqp_config, amqp_install):
    ...
    diff_settings = [key for key, value in current_settings.items() if amqp[key] != value]
    ...
    if diff_settings:
        ...
        prompt_msg = f"\nConfigure/create AMQP exchange 'amqp['exchange']'?"
...
def create_amqp_exchange(exchange_name, username, password, host, port, vhost,
                         use_ssl):
    """Checks if the specified AMQP exchange exists. If not, creates it
