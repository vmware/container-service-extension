...
def validate_amqp_config(amqp_dict):
    ...
    try:
        ...
        click.secho(f"AMQP Connection failed", fg='red')
        raise
    finally:
...
def validate_vcd_and_vcs_config(vcd_dict, vcs):
    ...
    types. Also checks that vCD and vcs are accessible.
    ...
    client.set_credentials(BasicLoginCredentials(vcd_dict['username'],
                                                 'System',
                                                 vcd_dict['password']))
    click.secho(f"Connected to vCloud Director as system administrator "
...
def validate_broker_config(broker_dict):
    ...
    if not default_exists:
        click.secho('Default template not found in listed templates', fg='red')
        raise ValueError()


def check_config(filepath, check_install=False, template='*'):
    """Handles `cse check` command.

    Checks that config file is correct. If 'check_install' flag is set, also
    checks that CSE is installed on vCD according to the config file.

    :param str filepath: file path to config file.
    :param bool check_install: determines whether or not to check that CSE is
        installed on vCD according to the config file.
    :param str template: specific template to check for. Default behavior
        checks all templates specified in config file.

    :raises KeyError: if config file has missing or extra properties.
    :raises ValueError: if the value type for a config file property
        is incorrect.
    :raises MissingRecordException: if CSE is not registered to vCD.
    :raises Exception: if AMPQ connection failed.
    :raises EntityNotFoundException: if catalog specified in config file cannot
        be found, or if template from config file cannot be found in catalog.
    """
    click.secho(f"Validating config file '{filepath}'", fg='yellow')
    config = get_validated_config(filepath)
    click.secho(f"Config file '{filepath}' is valid", fg='green')

    if not check_install:
        return

    click.secho(f"Validating CSE installation according to config file "
                f"'{filepath}'", fg='yellow')
    ...
    try:
        cse_info = ext.get_extension('cse', 'cse')
        if cse_info['enabled'] != 'true':
            click.secho("CSE is currently disabled", fg='red')
    except MissingRecordException:
        click.secho("CSE is not registered to vCD", fg='red')
        raise
    ...
    try:
        ...
        click.secho(f"Catalog '{catalog_name}' not found", fg='red')
        raise

    # check that templates exist in vCD
    for t in config['broker']['templates']:
        if template == '*' or template == t['name']:
            click.secho(f"Validating template '{t['name']}'", fg='yellow')
            try:
                org.get_catalog_item(catalog_name, t['catalog_item'])
                click.secho(f"Found template '{t['catalog_item']}' in "
                            f"catalog '{catalog_name}'", fg='green')
            except EntityNotFoundException:
                click.secho(f"Template '{t['catalog_item']}' not found in "
                            f"catalog '{catalog_name}'", fg='red')
                raise

    client.logout()
    click.secho("CSE installation is valid", fg='green')
