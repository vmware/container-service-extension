            ...
            if node_type == NodeType.NFS:
                ...
                if errors:
                    raise ScriptExecutionError(
                        f"Script execution failed on node {vm_name}:"
                        f"{script}:{errors}")
...
def get_master_ip(vapp):
    ...
    if errors:
        raise ScriptExecutionError(
            f"Script execution failed on master node {node_names}:"
            f"{script}:{errors}")
...
def join_cluster(vapp, template_name, template_revision, target_nodes=None):
    ...
    if errors:
        raise ScriptExecutionError(
            f"Script execution failed on worker node {node_names}:"
            f"{script}:{errors}")
...
def delete_nodes_from_cluster(vapp, node_names):
    ...
    if errors:
        raise ScriptExecutionError(
            ...
            f"{master_node_names}:{script}:{errors}")
    if result[0][0] != 0:
