...
def _override_client(ctx) -> None:
    ...
    """
    profiles = Profiles.load()
    server_api_version = profiles.get(CSE_SERVER_API_VERSION)
    ...
    if not server_api_version:
        ...
        server_api_version = sys_info.get(CSE_SERVER_API_VERSION)
        profiles.set(CSE_SERVER_API_VERSION, server_api_version)
        profiles.save()
    client = Client(
        profiles.get('host'),
