...
@click.group(name='pks-cluster', short_help='Manage PKS clusters')
@click.pass_context
def pks_group(ctx):
...
Examples
    vcd cse pks-cluster list
        Display clusters in PKS that are visible to the logged in user.
\b
    vcd cse pks-cluster list --vdc ovdc1
        Display clusters in PKS backed by 'ovdc1'.
\b
    vcd cse pks-cluster create mycluster
        Create a PKS cluster named 'mycluster'.
...
@pks_group.command('list',
                   short_help='Display clusters in PKS that are visible '
...
@pks_group.command('delete',
                   short_help='Delete a PKS cluster')
@click.pass_context
...
@pks_group.command('create', short_help='Create a PKS cluster')
@click.pass_context
@click.argument('cluster_name', required=True)
@click.option(
    '-v',
...
def cluster_create(ctx, cluster_name, vdc, node_count, org_name):
    """Create a PKS Kubernetes cluster (max name length is 25 characters)."""
    try:
...
@pks_group.command('resize',
                   short_help='Resize the PKS cluster to contain the '
...
@pks_group.command('config', short_help='Display PKS cluster configuration')
@click.pass_context
@click.argument('cluster_name', required=True)
@click.option(
    '-o',
...
@pks_group.command('info',
                   short_help='Display info about a PKS Kubernetes cluster')
@click.pass_context
@click.argument('cluster_name', required=True)
@click.option(
    '-v',
