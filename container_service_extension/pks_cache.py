...
class PksCache(object):
    ...
    __slots__ = ["orgs", "pks_accounts", "pvdcs",
                 ...
                 "org_pks_table", "pks_service_accounts_per_vc_info_table"]
    ...
    def __init__(self, orgs, pks_accounts, pvdcs):
        ...
        if orgs is not None and orgs[0]['name'] == 'None':
            ...
            org_pks_table = \
                self.__load_pks_service_accounts_per_org_per_vc(orgs,
                                                                pks_info_table)
    ...
    def get_pvdc_info(self, pvdc_id):
        """ Returns an immutable PvdcResourcePoolPathInfo object which
        has details of pvdc associated with this identifier. Details
        include datacenter name, cluster name and sub resource pool
        path for this provider vDC.
    ...
    def get_pks_account_details(self, org_name, vc_name):
        """Returns an immutable PksInfo object which has details
        of PKS account associated with the given organization and
        vCenter name.
    ...
    def __load_pks_accounts(self, pks_accounts):
        """Construct a dict to access PKS account information (account
        name, host, port, uaac and vc name) based on its account name,
        from the pks information obtained from configuration.

        :param list pks_accounts: array of dict, each representing PKS
        information in CSE PKS config.
    ...
    def __load_pks_service_accounts_per_vc(self, orgs, pks_accounts):
        ...
        :param list orgs: array of dict, each representing organization and
        its associated PKS accounts.
    ...
    def __load_pks_service_accounts_per_org_per_vc(self, orgs, pks_accounts):
        """Construct a dict to access PKS account information (account name, host,
        ...
        :param list orgs: array of dict, each representing organization and its
        associated PKS accounts.
        :param dict pks_accounts: dict of pks account name(key) and PKS
        information(value) in CSE PKS config.
        :return: dict of PKS information where key is the PKS account name and
        value is PksInfo object.
    ...
    def __load_pvdc_info(self, pvdcs_list):
        """Construct a dict to access pvdc information (datacenter, cluser
        and resource pool path) based on its identifier, from the pvdc
        information obtained from PKS configuration.

        :param list pvdcs_list: array of dict, each representing Pvdc
        information in CSE PKS config
...
class PvdcResourcePoolPathInfo(namedtuple("PvdcResourcePoolPathInfo",
                                          ...
                                          ' rp_path')):
    """
    Immutable class representing Provider vDC related information for
    PKS setup.
    """
    def __str__(self):
