...
class VcdBroker(abstract_broker.AbstractBroker):
    ...
    def update_cluster_acl(self, **kwargs):
        ...
        update_acl_entries = spec_body[shared_constants.ClusterAclKey.ACCESS_SETTING]  # noqa: E501

        # Get previous def entity acl
        cloudapi_client = vcd_utils.get_cloudapi_client_from_vcd_client(
            self.context.client)
        prev_user_acl_info = _get_all_def_ent_acl(cloudapi_client, cluster_id)

        try:
            updated_user_acl_level_dict = _update_native_def_entity_acl(
            ...
            curr_user_acl_info = _get_all_def_ent_acl(cloudapi_client,
                                                      cluster_id)
...
def _get_all_def_ent_acl(cloudapi_client, de_id):
