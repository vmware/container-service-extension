...
from container_service_extension.server_constants import \
    ...
    RemoteTemplateKey, SYSTEM_ORG_NAME, TemplateBuildParametersKey
...
def _install_template(client, remote_template_manager, template, org_name,
    ...
    temp_vm_name = f"{template[RemoteTemplateKey.OS].replace('.','')}" \
        f"-k8s{template[RemoteTemplateKey.KUBERNETES_VERSION].replace('.', '')}" \
            f"-{template[RemoteTemplateKey.CNI]}{template[RemoteTemplateKey.CNI_VERSION].replace('.','')}-vm"
    build_params = {
        TemplateBuildParametersKey.TEMPLATE_NAME: template[RemoteTemplateKey.NAME],
        TemplateBuildParametersKey.TEMPLATE_REVISION: template[RemoteTemplateKey.REVISION],
