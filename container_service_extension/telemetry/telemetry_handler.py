...
def user_action_recorder(cse_operation):
...
def record_user_action(cse_operation, status=OperationStatus.SUCCESS,
    ...
    if telemetry_settings['enable']:
        try:
            ...
            _send_data_to_analytics_server(payload, telemetry_settings)
        except Exception as err:
            LOGGER.warning(f"Error in recording user action information:{str(err)}")  # noqa: E501


def record_cse_operation_details(cse_operation, cse_params,
...
def _send_data_to_analytics_server(payload, telemetry_settings):
