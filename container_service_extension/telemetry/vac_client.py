...
class VacClient(object):
    ...
    def ingest_data(self, payload=None):
        ...
        if not self._collector_id:
            self._log_error("Invalid collector id", payload)
            return

        if not self._instance_id:
            self._log_error("Invalid instance id", payload)
            return
        ...
        try:
            ...
            self._log_error(err.response.text, payload)
        except Exception as err:
            self._log_error(err, payload)
        finally:
            if response:
                response.close()
    ...
    def _log_error(self, error_message, payload):
        self.LOGGER.error(f"{error_message}. "
                          f"Failed to ingest telemetry payload:{payload}")
