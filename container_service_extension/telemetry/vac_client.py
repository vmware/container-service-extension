...
INGEST_FAILED_MSG = "Failed to ingest telemetry payload"
...
class VacClient(object):
    ...
    def ingest_data(self, payload=None):
        ...
        if not self._collector_id:
            msg = f"Invalid collector id.{INGEST_FAILED_MSG}:{payload}"
            self.LOGGER.error(msg)
            return

        if not self._instance_id:
            msg = f"Invalid instance id.{INGEST_FAILED_MSG}:{payload}"
            self.LOGGER.error(msg)
            return
        ...
        try:
            ...
            msg = f"{err.response.text}.{INGEST_FAILED_MSG}:{payload}"
            self.LOGGER.error(msg)
        except Exception as err:
            msg = f"{err}.{INGEST_FAILED_MSG}:{payload}"
            self.LOGGER.error(msg)
        finally:
            if response:
                response.close()
